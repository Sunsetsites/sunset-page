        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5 mt-6">
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none border-b border-neutral-800/40 pb-6 " key={it.label + idx}>
                    <a
...
@media (max-width: 640px) { .sm-scope .staggered-menu-header { top: 0; left: 0; width: 100vw; padding: 24px; border-bottom: 1px solid rgba(23, 23, 25, 0.1); background: rgba(255, 255, 255, 0.92); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); z-index: 999; } .sm-scope .sm-panel-inner { gap: 1rem; } .sm-scope .staggered-menu-panel, .sm-scope .sm-prelayers { width: 100vw; } .sm-scope .staggered-menu-panel { right: 0; border-radius: 0; padding: 5.5rem 1.25rem 1.25rem; } .sm-scope .sm-panel-list { gap: 1rem; } .sm-scope .sm-panel-item { font-size: clamp(1.55rem, 8.5vw, 2.1rem); } .sm-scope .sm-contact-block { padding-top: 1.5rem; } .sm-scope .sm-socials { padding-top: 1rem; } .sm-scope .sm-socials-link { font-size: 0.7rem; } }
      `}</style>
    </div>
  );
};
